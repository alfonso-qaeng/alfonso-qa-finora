...
async function cmdTestGet(
  ...
  console.log(`Type: ${test.testType.name}`);
  const testStatus =
    typeof test.jira.status === 'object' && test.jira.status !== null
      ? test.jira.status.name
      : test.jira.status || 'Unknown';
...
async function cmdTestList(flags: Record<string, string | boolean>): Promise<void> {
  ...
  result.getTests.results.forEach((t: TestResult) => {
    ...
    const status =
      typeof rawStatus === 'object' && rawStatus !== null ? rawStatus.name : rawStatus || 'Unknown';
...
async function cmdExecGet(
  ...
  console.log(`Summary: ${exec.jira.summary}`);
  const execStatus =
    typeof exec.jira.status === 'object' && exec.jira.status !== null
      ? exec.jira.status.name
      : exec.jira.status || 'Unknown';
...
async function cmdExecList(flags: Record<string, string | boolean>): Promise<void> {
  ...
  result.getTestExecutions.results.forEach((e: TestExecutionResult) => {
    const eStatus =
      typeof e.jira.status === 'object' && e.jira.status !== null
        ? e.jira.status.name
        : e.jira.status || 'Unknown';
...
async function cmdPlanList(flags: Record<string, string | boolean>): Promise<void> {
  ...
  result.getTestPlans.results.forEach((p: TestPlanResult) => {
    const pStatus =
      typeof p.jira.status === 'object' && p.jira.status !== null
        ? p.jira.status.name
        : p.jira.status || 'Unknown';
...
async function cmdSetGet(
  ...
  console.log(`Summary: ${testSet.jira.summary}`);
  const setStatus =
    typeof testSet.jira.status === 'object' && testSet.jira.status !== null
      ? testSet.jira.status.name
      : testSet.jira.status || 'Unknown';
...
async function cmdSetList(flags: Record<string, string | boolean>): Promise<void> {
  ...
  result.getTestSets.results.forEach((s: TestPlanResult) => {
    const sStatus =
      typeof s.jira.status === 'object' && s.jira.status !== null
        ? s.jira.status.name
        : s.jira.status || 'Unknown';
...
async function cmdBackupExport(flags: Record<string, string | boolean>): Promise<void> {
  ...
  if (onlyWithData && testsData.length < totalTests) {
    log.success(
      `Exported ${testsData.length}/${totalTests} tests (${totalTests - testsData.length} skipped - no Xray data)`
    );
  } else {
  ...
  for (const test of backup.tests) {
    ...
    if (syncMode) {
      ...
      if (existingTest) {
        ...
        if (dryRun) {
          console.log(
            `  [DRY] Would sync: ${test.originalKey} (${test.testType}) - found via ${source}`
          );
          testsUpdated++;
          continue;
        }
        ...
        try {
          ...
          const hasXrayData =
            (test.testType === 'Cucumber' && test.gherkin) ||
            (test.testType === 'Generic' && test.unstructured);
          const needsTypeChange =
            hasXrayData &&
          ...
          if (needsTypeChange) {
            log.dim(
              `  Changing test type: ${existingTest.testType || 'Manual'} → ${test.testType}`
            );
